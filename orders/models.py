...
# Устанавливаем точность Decimal
getcontext().prec = 10
...
class Order(models.Model):
    ...
    customer = models.ForeignKey(
        Customer,
        on_delete=models.PROTECT,  # Изменено на PROTECT для безопасности данных
        related_name='orders',
        verbose_name='Клиент'
    )
    ...
    def confirm(self):
        """Подтверждение заказа с проверкой наличия товаров"""
    ...
    def cancel(self, restock=True):
        """Отмена заказа с возможностью возврата товаров на склад"""
...
class OrderItem(models.Model):
    ...
    product = models.ForeignKey(
        'products.Product',
        on_delete=models.PROTECT,  # Защита от удаления товара с историей заказов
        verbose_name='Товар'
    )
    ...
    def save(self, *args, **kwargs):
        """Переопределение сохранения с автоматическим расчетом цены"""
